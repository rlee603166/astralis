...
class Astralis:
    ...
    Core Functions
    """
    async def run(self, user_query: str, session_id: str) -> AsyncGenerator[Dict[str, Any], None]:
        """
        ...
        self.context['history'] = await self._load_history(session_id)
        self.context["user_query"] = user_query

        self.context.pop('needs_clarification', None)
        self.context.pop('clarification_question', None)

        print(f"[RUN START]: Session {session_id}, Query: '{user_query}', History Len: {len(self.context['history'])}")
        ...
        try:
            while True:
                ...
                if self.context.get('needs_clarification'):
                    ...
                    await self._save_history(session_id, self.context['history'])
    ...
    async def _generate_thought(self):
        ...
        formatted_hist = self._formatted_history(history)
    ...
    async def _generate_final_users(self, final_response):
        history = self.context.get('history', [])
        formatted_hist = self._formatted_history(history) + final_response
    ...
    def _formatted_history(self, history: List[Dict[str, Any]]) -> str:
        if not history:
